...
QEMU_BINARY=$LOCALPREFIX/bin/qemu-system-x86_64
...
#FASTHDDEV="-hda $BOOTDISK -boot c"
FASTHDDEV="-drive id=disk,file=$BOOTDISK,if=none -device ahci,id=ahci -device ide-hd,drive=disk,bus=ahci.0"
#FASTHDDEV="-drive file=$BOOTDISK,if=virtio"
SND="-device intel-hda"
NETDRIVER="e1000"
QEMU_ACCEL="tcg"
QEMU_VNC="-vnc 127.0.0.1:0"
QEMU_CMD="$QEMU_BINARY -smp $QEMU_SMP -device $NETDRIVER,netdev=eth0 -netdev vmnet-bridged,ifname=eth0,id=eth0,ifname=$NETWORKDEV -accel $QEMU_ACCEL -M pc -m $QEMU_MEM -nographic $FASTHDDEV $SND $QEMU_VNC -boot c "

TERM_OPEN="osascript -e 'tell app "Terminal" to do script "$QEMU_CMD"'"

URL="https://download.freebsd.org/releases/amd64/amd64/ISO-IMAGES/14.2/FreeBSD-14.2-RELEASE-amd64-dvd1.iso"
JOBS=80
