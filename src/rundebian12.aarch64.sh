...
#FASTHDDEV="-device virtio-blk-pci,drive=hd0,id=hd0pci -drive if=virtio,file=$BOOTDISK,format=qcow2"
#FASTHDDEV="-drive if=none,file=$BOOTDISK,id=hd0 -device virtio-blk-device,drive=hd0"
FASTHDDEV="-hda $BOOTDISK"
SND="-device intel-hda"
NETDRIVER="e1000"
QEMU_ACCEL="hvf"
QEMU_VNC="-vnc 127.0.0.1:1"
QEMU_CMD="$QEMU_BINARY -smp $QEMU_SMP -device $NETDRIVER,netdev=eth0 -netdev vmnet-bridged,ifname=eth0,id=eth0,ifname=$NETWORKDEV -M virt -cpu cortex-a72 -accel $QEMU_ACCEL -M virt -bios $QEMU_BIOS -m $QEMU_MEM -nographic $FASTHDDEV $SND $QEMU_VNC"

TERM_OPEN="osascript -e 'tell app "Terminal" to do script "$QEMU_CMD"'"

URL="https://chuangtzu.ftp.acc.umu.se/debian-cd/current/arm64/iso-dvd/debian-12.10.0-arm64-DVD-1.iso"
JOBS=80
...
function backup_image(){
    if [[ -f $BACKUPDIR ]]; then
     echo "backing up images to $BACKUPDIR"                                         cp -v $BOOTDISK $BACKUPDIR                                                
     echo "done"
    else
	
	echo "$BACKUPDIR not found, creating $BACKUPDIR"
	mkdir $BACKUPDIR
	echo "backing up"
	tar cfv $BOOTDISK.tar $BACKUPDIR/backup-`date`.tar
    fi
}
